...
Notes and tips
	- Railway free tier provides a generous starter plan; ensure your MongoDB Atlas has an IP allowlist or set to allow access from Railway (Atlas may allow access from anywhere if you set it so, but it's safer to configure connection security).
	- If you prefer Heroku, Render, or Fly, deployment steps are very similar: set env vars and point the deploy root to `server`.
